clc;clear;
close all;
% 接收事件相机冰袋识别结果的kuka的视觉引导程序
% 使用了KST类
% 将ivgep3的 接收二维码角点并自己计算中心点与旋转角度 改成 直接接收中心点坐标和旋转角度
% 收数据改成只收一次
...
camParam = IBVS_createCamera('davis');

% 机器人末端到相机前端（事件相机取本体）距离
Lcamera = 0.11;
% 希望预留的相机与物体距离
Loc = 0.3 + 0.05; % 事件相机额外加上镜头长度
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
disp('Have gotten image infomation.');
centerpixel = meanCandRz(1:2); EEF_Rz_plus = meanCandRz(3)/180*pi;
