...
jPos = mat2cell(qinit,[1 1 1 1 1 1 1])';
...
massOfTool=0.626; % the mass of the tool attached to flange in Kg
cOMx=0; % X coordinate of the center of mass of the tool in (mm)
cOMy=0; % Y coordinate of the center of mass of the tool in (mm)
cOMz=100; % Z coordinate of the center of mass of the tool in (mm)
% cStiness=900; % cartizian stifness
% rStifness=80; % rotational stifness
% nStifness=50; % null space stifness

cStiness=900*2; % cartizian stifness
rStifness=80*2; % rotational stifness
nStifness=50*2; % null space stifness
...
A=0.15; % motion constants, amplitude of motion
...
% real time plot handles
 ...
 % tawVec_save=zeros(7,0);
 eef_pose_save = zeros(6,0);
 jnt_save = zeros(7,0);
 plotHandle=[];
 for i=1:7
...
while(dt<duration)
 %% perform trajectory calculation here
  ...
  jnt = iiwa.sendJointsPositionsGetActualJpos(jPosCommand);
  eef_pos = iiwa.sendJointsPositionsGetActualEEFpos(jPosCommand);
  mjnt = cell2mat(jnt);
  meef_pos = cell2mat(eef_pos);
  tawTemp=zeros(7,1);
  for i=1:7
      tawTemp(i)=taw{i};
  ...
  % tawVec_save=[tawVec_save, tawTemp];
  jnt_save = [jnt_save; mjnt];
  eef_pose_save = [eef_pose_save; meef_pos];
  % update plots data
  for i=1:6
